...
def create_traffic_matrix (nodes , rate ):
    ...
    df = pd. DataFrame (\
    float ( rate ) , index =s.index , columns =s.index , dtype =s. dtype )
...
# after having gotten the node pairs requested for connection we stream them
...
# Plot
    ...
    # the capacity is obtained by summing all channels bitrate of a path in every connection
    ...
    print ("bitrate for all paths between two connected nodes\n", df)
    #plot3Dbars (t)
